      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 w-full">
          {latestProducts?.map((product) => (
            <Link to={`/product/${product.id}`} key={product.id} className="block group w-full">
              <div
                className="bg-white rounded-lg shadow-md overflow-hidden relative group-hover:shadow-xl transition-all duration-300 h-full border border-gray-100 w-full"
              >
