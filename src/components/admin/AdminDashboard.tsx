...
const AdminDashboard = () => {
  ...
  return <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-12">
        ...
        {categoryOrder.map(category => <div key={category} className="mb-8">
            <h2 className="text-xl font-semibold text-gray-700 mb-4 border-b border-gray-200 pb-2">
              {category}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {groupedCards[category]?.map((card, index) => <motion.div key={card.title} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
                  ...
                  <Link to={card.link} className="block h-full">
                    <div className="bg-white rounded-lg shadow-sm p-6 h-full hover:shadow-md transition-shadow">
                      <div className="relative">
                        ...
                        {card.notificationCount > 0 && <NotificationBadge count={card.notificationCount} className="absolute -top-2 -right-2" />}
                      </div>
