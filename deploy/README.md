...
- **URL principale** : http://84.234.18.206
- **Port** : 80 (HTTP standard)
- **Protocole** : HTTP (HTTPS avec certificat SSL optionnel)
...
1. **Pointage DNS** : Pointez votre domaine vers `84.234.18.206`

2. **Modification Nginx** :
```bash
sudo nano /etc/nginx/sites-available/big-market
# Remplacer: server_name 84.234.18.206 _;
# Par: server_name votre-domaine.com;
sudo systemctl reload nginx
```
...
**🎉 Votre site Big Market est maintenant déployé et accessible sur http://84.234.18.206 !** 